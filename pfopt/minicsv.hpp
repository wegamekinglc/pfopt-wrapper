...
// version 1.3   : fixed: to when reading the last line and it does not have
// linefeed
//                 added: skip_1st_line and skip_line functions to ifstream
//                 class
...
// version 1.7   : You MUST specify the escape/unescape string when calling
// set_delimiter. Option to surround/trim string with quotes
...
// version 1.7.5 : Add terminate_on_blank_line variable. Set to false if your
// file format has blank lines in between.
// version 1.7.6 : Ignore delimiters within quotes during reading when
// enable_trim_quote_on_str is true;
...
// version 1.7.10 : separator class for the stream, so that no need to call
// set_delimiter repeatedly if delimiter keep changing
// version 1.7.11 : Fixed num_of_delimiters function: do not count delimiter
// within quotes
// version 1.8.0  : Add meaningful error message for data conversion during
// reading
...
// version 1.8.4  : Add NChar class and fix error of no delimiter written for
// char type.
...
namespace mini {
    ...
    namespace csv {
        ...
        class sep // separator class for the stream, so that no need to call
                  // set_delimiter
        {
        public:
        ...
        public:
            ifstream(const std::string& file = "")
                : str(""), pos(0), delimiter(","), unescape_str("##"), trim_quote_on_str(false), trim_quote('\"'),
                  trim_quote_str(1, trim_quote), terminate_on_blank_line(true), quote_unescape("&quot;"),
                  has_bom(false), first_line_read(false), filename(""), line_num(0), token_num(0) {
                open(file);
            }
            ifstream(const char* file) { open(file); }
            ...
            void read_bom() {
                ...
                if (tt[0] == (char)0xEF || tt[1] == (char)0xBB ||
                    tt[2] == (char)0xBF) // not the correct BOM, so reset the pos to
                                         // beginning (file might not have BOM)
                    has_bom = true;
            ...
            void close() { istm.close(); }
            bool is_open() { return istm.is_open(); }
            ...
            std::string const& get_delimiter() const { return delimiter; }
            std::string const& get_unescape_str() const { return unescape_str; }
            void skip_line() {
                if (!istm.eof()) {
            ...
            std::string get_rest_of_line() const { return str.substr(pos); }
            const std::string& get_line() const { return str; }
            void enable_terminate_on_blank_line(bool enable) { terminate_on_blank_line = enable; }
            bool is_terminate_on_blank_line() const { return terminate_on_blank_line; }
            std::string error_line(const std::string& token, const std::string& function_site) {
                std::ostringstream is;
                is << "csv::ifstream Conversion error at line no.:" << line_num << ", filename:" << filename
                   << ", token position:" << token_num << ", token:" << token << ", function:" << function_site;
        ...
        public:
            ...
            ofstream(const char* file) { open(file); }
            void open(const std::string& file) {
                if (!file.empty())
                    open(file.c_str());
            }
            ...
            void flush() { ostm.flush(); }
            void close() { ostm.close(); }
            bool is_open() { return ostm.is_open(); }
            ...
            std::string const& get_delimiter() const { return delimiter; }
            std::string const& get_escape_str() const { return escape_str; }
            void set_after_newline(bool after_newline_) { after_newline = after_newline_; }
            bool get_after_newline() const { return after_newline; }
...
template <typename T> mini::csv::ifstream& operator>>(mini::csv::ifstream& istm, T& val) {
...
template <> inline mini::csv::ifstream& operator>>(mini::csv::ifstream& istm, std::string& val) {
    val = istm.get_delimited_str();

    return istm;
}

template <> inline mini::csv::ifstream& operator>>(mini::csv::ifstream& istm, mini::csv::sep& val) {
...
template <> inline mini::csv::ifstream& operator>>(mini::csv::ifstream& istm, char& val) {
...
template <typename T> mini::csv::ofstream& operator<<(mini::csv::ofstream& ostm, const T& val) {
...
template <> inline mini::csv::ofstream& operator<<(mini::csv::ofstream& ostm, const std::string& val) {
    if (!ostm.get_after_newline())
        ostm.get_ofstream() << ostm.get_delimiter();
...
template <> inline mini::csv::ofstream& operator<<(mini::csv::ofstream& ostm, const mini::csv::sep& val) {
    ostm.set_delimiter(val.get_delimiter(), val.get_escape());

    return ostm;
}

template <> inline mini::csv::ofstream& operator<<(mini::csv::ofstream& ostm, const char& val) {
    if (val == NEWLINE) {
    ...
    namespace csv {
        ...
        public:
            istringstream(const char* text)
                : str(""), pos(0), delimiter(","), unescape_str("##"), trim_quote_on_str(false), trim_quote('\"'),
                  trim_quote_str(1, trim_quote), terminate_on_blank_line(true), quote_unescape("&quot;"), line_num(0),
                  token_num(0) {
                istm.str(text);
            }
            void enable_trim_quote_on_str(bool enable, char quote, const std::string& unescape = "&quot;") {
            ...
            std::string error_line(const std::string& token, const std::string& function_site) {
                std::ostringstream is;
                is << "csv::istringstream conversion error at line no.:" << line_num << ", token position:" << token_num
        ...
        public:
            ...
            std::string get_text() { return ostm.str(); }
            void escape_and_output(std::string src) {
                ostm << ((escape_str.empty()) ? src : replace(src, delimiter, escape_str));
            }
            void escape_str_and_output(std::string src) {
...
template <typename T> mini::csv::istringstream& operator>>(mini::csv::istringstream& istm, T& val) {
    const std::string& str = istm.get_delimited_str();
...
template <> inline mini::csv::istringstream& operator>>(mini::csv::istringstream& istm, std::string& val) {
    val = istm.get_delimited_str();

    return istm;
}

template <> inline mini::csv::istringstream& operator>>(mini::csv::istringstream& istm, mini::csv::sep& val) {
    istm.set_delimiter(val.get_delimiter(), val.get_escape());
...
template <> inline mini::csv::istringstream& operator>>(mini::csv::istringstream& istm, char& val) {
    const std::string& src = istm.get_delimited_str();
...
template <typename T> mini::csv::ostringstream& operator<<(mini::csv::ostringstream& ostm, const T& val) {
    if (!ostm.get_after_newline())
...
template <> inline mini::csv::ostringstream& operator<<(mini::csv::ostringstream& ostm, const std::string& val) {
    if (!ostm.get_after_newline())
        ostm.get_ostringstream() << ostm.get_delimiter();
...
template <> inline mini::csv::ostringstream& operator<<(mini::csv::ostringstream& ostm, const mini::csv::sep& val) {
    ostm.set_delimiter(val.get_delimiter(), val.get_escape());

    return ostm;
}

template <> inline mini::csv::ostringstream& operator<<(mini::csv::ostringstream& ostm, const char& val) {
    if (val == NEWLINE) {
